...
- **A vacuous *category* has three possible causes, and only one of them is permanent.** Distinct from the four vacuity *modes* above, which are ways a gate fails to discriminate: this is why a category had nothing to score at all. `synthesized-endpoint` was misattributed across two turns because the distinction was not written down — 0021 blamed the driver, the driver was then fixed, and the category did not move, because the real cause was a third thing.
  ...
  Within the three, the third is the one that hides, because fixing the driver makes the input look right while the number stays put. The discriminator is the last column: **is the input empty, or is the input full and the output still empty?** `flows/skipped-controls.json` holds 60 controls and `synthesized-endpoint` is still vacuous, because `packages/infer` has no code path that emits `bound-from-control` — §7.6 was never written. State the cause whenever a category is reported vacuous; "vacuous" without one sends the next reader to the wrong package.
