...
- **A score is named after the stage that writes the file, never after the stage that produced the content — and those differ.** Infer was run four times against one capture, once with each of its pieces disabled, and graded each time: entity dedup changed the model from four entities to seven, the token pass changed sixteen colours to none, and **no graded category moved for any of them**. Every metric reads `model.operations`, which infer transcribes from `capture/network/endpoints.json`. That is not a misplaced stage and the work must not be moved on the strength of it: §5 puts response schemas in the *capture* artifact ("for each endpoint, infer a JSON Schema across all observed responses"), and §6's "deterministic, no LLM" does not exclude them, because a schema union over observed bodies is deterministic. What it means is narrower and worse — **the grader's field-level categories score a capture artifact**, so 0015's premise that they measure infer was wrong, and twelve numbers badged as infer's are largely capture's. Two consequences. Before reading a score as evidence about a stage, disable a piece of that stage and check the number moves; a metric no variant can shift is measuring something upstream. And a metric can be structurally unable to fail: `endpoint-identity.recall` is over the observed list, `model.operations` is a total `.map` over that same list, so it reads 1.000 for any infer that transcribes and 1.000 is not a result. Ask what edit would move a metric before quoting it.
