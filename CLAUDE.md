...
- **Test a structural property structurally, never through a numeric proxy.** The rule that a claim on the model must be specific enough was first written as "at least two segments", which reads like a structural check and is not: two segments is a fact about a string, and `entities.fields` has two while covering twelve leaves. Replaced by "the claim must terminate at a schema leaf", which is the property itself. Same family as `endsWith` on a path — a stand-in that correlates with the thing you mean, until the day it does not — and the sixth occurrence of it here. Whenever a threshold on a count is standing in for a shape, ask what the shape is and assert that.
