...
- **The seed must populate every collection-valued field of every entity the environment poses tasks about**, and the same requirement binds the *fixture* seed a capture is taken against. A field nobody ever populated is captured as `null`, so the response schema says `null`, so the mock store seeds nothing there and every §10 validator that reads it is validating against a hole. Measured on Vikunja: with `assignees`, `labels`, `reminders` and `attachments` unpopulated, those four fields alone owned **63 of 126** response-recall misses, and the metrics read as inference quality. **A field left out does not stay where it was** — `attachments`, deferred one turn, went from 29 misses to 46, because a newly-matched endpoint declared it too. Seed the collection or record the gap; there is no third option in which the number means anything (0051, 0053).
...
- **A committed sabotage is anchored on the expression it changes, never on its neighbourhood.** `git apply` matches context lines, so two patches rotted in a single turn from ordinary product commits that moved those lines while the code each targeted sat untouched — a comment growing above a call, an import joining a block. Neither defect changed. That tax compounds with the patch count, so a sabotage is `{file, find, replace}` where `find` is the expression carrying only as much surrounding text as **uniqueness** requires; converted mechanically by growing context from the changed span until the whole edit is unambiguous, the median anchor is 2 lines. Two hard failures and both say something the old form could not: `absent` means the *target* changed rather than its neighbourhood, and `ambiguous` — two matches — fails rather than editing the first. Two further rules came out of getting the conversion wrong, and both generalize past this harness — they are the next two entries.
- **An operation that restores more than it changed is not a safer undo; it is a different operation.** A revert reverses *what was applied*, and reaching for a bigger hammer widens the blast radius rather than the safety margin. `git checkout -- <file>` looks like the cautious choice against a hand-written inverse and is not: it restores **committed** bytes, so it also discards every uncommitted change to that file that the operation never made. Measured: the sabotage harness edits `scripts/sabotage.mjs` itself (`build-residue-ignores-javascript`), so the "safe" revert destroyed the working copy of the very rewrite that introduced it, and the `git apply -R` it replaced had the weaker and *correct* semantics. The tell is a scope mismatch between the two directions — the do names an expression and the undo names a file, a directory, a database, a container. Ask what the operation touched, and require the inverse to touch exactly that; where the inverse genuinely cannot be that narrow, the difference is a **cost to declare**, not a detail. This is the question the probe-pass reset (0043 §4) has to answer before it is built: recreating the container undoes a probe's writes and also resets id sequences, clocks and seed state, which is a different starting condition from the one the crawl legitimately built up.
- **A converter's acceptance test is the real predicate, never a re-implementation of part of it.** Whatever consumes the output already encodes what "valid" means; a converter that re-derives the easy half of that in its own terms will pass things the consumer then rejects — and it will do so silently, because the converter reported success. Measured: the patch-to-semantic-edit converter checked "find is unique" and stopped there, so five pure deletions came out with an empty `replace` and nothing for the undo to anchor on — a case `assessSemanticEdit` names (`replacement-absent`) and the converter had no concept of. The fix is not a longer checklist: call the consumer's own predicate on each output and fail the conversion on its verdict.
...
- **A number drawn from a non-deterministic pass is reported with its spread, or with the run that produced it — never as a bare figure.** The companion to the rule above, pointed downstream: that one governs whether a *gate* may assert stability, this one governs every number quoted out of a pass already known to move. A figure with no draw attached is a point estimate that the next reader will treat as a property, and the further it travels from the run the more it looks like one. **Measured:** the probe pass writes `flows/skipped-controls.json`, whose control count read 84 · 83 · 79 across three completed crawls of one pinned digest, and §7.6 reads that file — so `considered`, every per-rung tally derived from it, and `synthesized-endpoint`'s denominator are all draws. The manual's own fix is the template and it is a *shape*, not a correction: state the property that holds (`skipped-controls.json` is **populated and unconsumed**) and let the count live in the run report beside the run. Where the number must be quoted, quote the series. `BindingTally.drawnFrom` carries the crawl for the same reason a `NarrowingRecord` carries its counts — the evidence travels with the claim, so the figure cannot be lifted away from what makes it one.
- **An instrument that changes its subject measures a third thing, and the tell is a prediction failing rather than a number looking wrong.** Three instances, so it is a rule rather than an anecdote. The **held-container control** (0032 §2.1) held the target still to separate our timing from its state — while the crawl mutates the target, so two held-container crawls differed on *more* paths (43) than two fresh ones (34). The **contaminating write** (0043 §2): probe *k* reads probe *k−1*'s writes, so the fired-control count is a function of position in the loop, 9 of 18 then 3 of 14 then 8 of 24 down one run. The **seed** (0051, 0053): enriching one task added interactive widgets a bare task does not have, so probes fired them, a new endpoint matched, and every denominator the prediction had called fixed moved — three of five predictions failed for a cause the prediction did not contain. None was visible by reading the instrument, and the first two were found only because a number moved the wrong way. So: **the identity of what an instrument runs against is part of the measurement's identity, and is recorded.** `manifest.seedState` carries the seed program's digest and the image it ran against, derived rather than declared and recomputed by the schema; a comparison between two gradings taken under different seed ids **refuses** rather than rendering a delta. Ask what the act of measuring changed, and record enough that a later comparison across the change can be refused by something other than memory.
- **A rate is not a result until its movement is decomposed, and the decomposition is three numbers.** `field-type` went 0.9000 → 0.9433 and cleared its gate with its miss count **byte-identical at 25**, on a denominator that went 250 → 441: a gate cleared entirely by growth, reported as an improvement for a turn. So every rate carries its **absolute miss count** — `denominator − numerator` where higher is better, the numerator where lower is better, and the direction is read from the gate or *declared* for the reported-only metrics, never defaulted. Two findings the grader prints, and both are deliberately wider than the case that motivated them, because the record holds instances the narrow rule passes: **a gate crossed while the denominator changed at all** — in *either* direction, no threshold, nothing to tune, since a gate crossed on a population that is not the same population is not a like-for-like pass; and **a rate that moved against its own miss count**, in either direction, since an improvement the misses do not back is a pass nobody earned and a regression they do not back is a scare nobody caused. Swept over every before/after pair in the decision record: **7 of 13 movements carry a finding**, in 0025, 0045/0046 and 0051 — including one where the rate *fell* while misses stayed byte-identical at 325, read as a regression in two documents. A rule written against the single case its author had in mind is the vacuity mode this repository keeps finding; write it against the class and sweep the record for the rest.
...
- **A producer with no consumer is the vacuity table's third cause at the top level, and an empty artifact is how it hides.** §13's table asks whether a *category* is empty because the input is empty or because the input is full and the output still is. Asked of the **product** rather than of a metric, it found four instances in one sweep, every one of them a deliverable §1 or §5 names. `GAPS.md` had a schema, a producer filling 43 records per run, three docstrings describing how it would be aggregated, and the literal text `_No runs recorded yet._` for the life of the project — the consumer was never written and no gate was looking at the file. The sweep that followed found three more, all a hardcoded `[]` in infer's model assembly: **`fonts` (70 `@font-face` descriptors captured, 0 emitted), `assets` (46 entries, 2.79 MB), `behaviours` (113 fired controls, 122 traces)**. And the same shape one level up in the *record*: decisions 0036 and 0042 were made, landed in code, rewrote this file — and their documents were never written while eight source sites cited them. **The tell is that an empty output is how all four causes spell themselves**, so `assets: []` reads exactly like the enum ladder correctly declining, and nothing distinguishes them without the input count beside it. So each declared hole carries the capture-side count it is measured against, declaring one whose input is *also* empty fails (that is a target or driver limitation and belongs elsewhere), and a declared hole that starts assembling fails until the row is removed. Ask of every artifact the spec names: does something produce it, does something consume it, and does any gate read the thing itself rather than the intent to write it. **The sweep's scope is the stages that run** — codegen, envkit and the CLI are scaffolds, so `tokens.json`, `verify/history.jsonl` and the control plane are not instances of this, they are stages nobody has built; the discriminator is a producer that already executes.
- **A figure used in an argument carries an assertion that fails when it moves — or names the fixture it describes.** Two classes and only one is pinnable. **Schema-derived** figures are properties of committed code — leaf counts, claim joins, table sizes — and take an *exact* assertion, never a floor: `expect(leaves.length).toBeGreaterThan(100)` is what let **421 leaves / 384 consumed / 267 unscored** stand through two documents and an approval while the true numbers were 318 / 290 / 207, because a floor cannot catch a count that moves *downward*. **Fixture-derived** figures are properties of a captured instance — endpoints, fields, controls, flows — and cannot be pinned at all; they carry the capture, run or pinned instance they were measured against, which is §13's spread-or-run rule pointed at documents instead of reports. Swept: of 41 load-bearing figures across the decision record, 9 carried an assertion, and **every unpinned one was fixture-derived except 421** — so the only pinnable class was exactly the class that had drifted. The failure is specific rather than general: a figure quoted in a document becomes a fact by being read, and the further it travels from the run the more it looks like a property of the system rather than of one measurement. Where a figure turns out to be unreproducible, **mark it rather than reconstruct it** — the original was true about the instance it measured, and a present-day equivalent replaces a true statement about the past with a guess.
