...
- **A stage boundary is a claim about who does the judgement, and it can be measured.** Infer was run four times against one capture, once with each of its pieces disabled, and graded each time: entity dedup changed the model from four entities to seven, the token pass changed sixteen colours to none, and **no graded category moved for any of them**. Every metric reads `model.operations`, which infer transcribes from `capture/network/endpoints.json` — so response-schema inference, which is §7.5's work, is being done in §6's stage, and the first real score is largely a score of capture. The fix is not to move the metrics onto what infer happens to produce; that is fitting the measurement to the thing measured. Either the work moves or the contract says why it should not. Whenever a stage's output is scored, ask which stage actually produced the thing being scored.
