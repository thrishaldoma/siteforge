...
Seven gates carry their sabotage as a committed patch under `sabotage/`.
...
- The sabotage patches gate on `pnpm test` rather than on a rung, so the
  statesProbed 2→3 measurement is pinned by a unit test on
  `selectorClassNames` rather than by the rung itself. A rung-level assertion on
  the exact count would be stronger and costs 22 seconds per sabotage.
