...
first real score is largely a score of the capture stage, and the parts of infer
that exercise §7's judgement — the data model, the narrowing ladder applied to
entity fields, the component extraction, the token clustering — are invisible to
every metric the grader has.

That is not an argument for moving the metrics; that would be fitting the
measurement to the thing measured, which is the failure 0015 §0 ordered the
steps to prevent. It is a statement about where the stage boundary currently
sits: response-schema inference is §7.5's work being done in §6's stage, and
the grader was built to score §7. Either the work moves or the contract says why
it should not, and that is a decision to take deliberately rather than as a side
effect of a scoring run — the grader is pinned (0020) and this note is the
place the argument belongs.
...
Bad, as expected, and informative in specific ways. `response-field-presence`
